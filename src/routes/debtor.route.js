...
const { debtorController: controller } = require('../controllers');
const { debtorValidation: validation } = require('../validations');
 ...
 * tags:
 *   name: Debtors
 *   description: Operaciones CRUD para deudores
 */
...
router.route('/:id/balance')
  .get(validate(validation.balance), controller.getBalance)
  .all(methodNotAllowed);
